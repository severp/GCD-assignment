...
all_activityID <- mutate(all_activityID, index = 1:10299)#index needed for sorting
...
names(activity) <- c("activityID", "index", "activity")
activity <- arrange(activity, index)
activity <- select(activity, -activityID, -index)
